...
var (
	...
	cacheZeroArticleIdPrefix = "cache:zero:article:id:"
)
...
func (m *defaultArticleModel) Delete(ctx context.Context, id int64) error {
	zeroArticleIdKey := fmt.Sprintf("%s%v", cacheZeroArticleIdPrefix, id)
...
func (m *defaultArticleModel) FindOne(ctx context.Context, id int64) (*Article, error) {
	zeroArticleIdKey := fmt.Sprintf("%s%v", cacheZeroArticleIdPrefix, id)
	var resp Article
	err := m.QueryRowCtx(ctx, &resp, zeroArticleIdKey, func(ctx context.Context, conn sqlx.SqlConn, v any) error {
...
func (m *defaultArticleModel) FindByTitle(ctx context.Context, title string) (*Article, error) {
	zeroArticleIdKey := fmt.Sprintf("%s%v", cacheZeroArticleIdPrefix, title)
	var resp Article
	err := m.QueryRowCtx(ctx, &resp, zeroArticleIdKey, func(ctx context.Context, conn sqlx.SqlConn, v any) error {
		query := fmt.Sprintf("select %s from %s where `title` = ? limit 1", articleRows, m.table)
		return conn.QueryRowCtx(ctx, v, query, title)
	})
	switch err {
	case nil:
		return &resp, nil
...
func (m *defaultArticleModel) Insert(ctx context.Context, data *Article) (sql.Result, error) {
	zeroArticleIdKey := fmt.Sprintf("%s%v", cacheZeroArticleIdPrefix, data.Id)
	ret, err := m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		query := fmt.Sprintf("insert into %s (%s) values (?, ?, ?)", m.table, articleRowsExpectAutoSet)
		return conn.ExecCtx(ctx, query, data.Id, data.Title, data.Content)
	}, zeroArticleIdKey)
	return ret, err
}
...
func (m *defaultArticleModel) formatPrimary(primary any) string {
	return fmt.Sprintf("%s%v", cacheZeroArticleIdPrefix, primary)
}
